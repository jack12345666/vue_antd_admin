       ...
       <a-form style="margin: 10px 5px;" @submit="handleSubmit" :form="form">
           ...
           <a-row :gutter="15">
            ...
            <div class="imgList" v-if="mainImg.length > 0">
                ...
                <upload :defaultImgList="mainImg" ref="img"/>
            </div>
            <div class="imgList">
                <div class="tips">最多上传6张厂房图片</div>
                <upload :defaultImgList="imgList" :limit="6" ref="uploadImg"/>
            </div>
...
import { updateLease, getLeaseDetail, getAreaList } from '../../api/lease'
export default {
    data() {
        return {
  ...
  created() {
   ...
   methods: {
    ...
    async submitInfo(values) {
        let rsp = await updateLease(values)
        if(rsp.code === 0) {
     ...
     async fetchLeaseInfo() {
         ...
         if(rsp.code === 0) {
            ...
            this.form.setFieldsValue({
                ...
                priceLow: this.leaseInfo.priceLow,
                priceHigh: this.leaseInfo.priceHigh,
                ...
                status: this.leaseInfo.status
            })
            if(this.leaseInfo.streetList.length > 0) {
               let arr = []
               this.leaseInfo.streetList.forEach(item => {
                 arr.push({
                   key: item.areaId,
                   label: item.areaName
                 })
            ...
            this.mainImg = [{uid: +new Date(), url: imgFormat(this.leaseInfo.mainPic), path: this.leaseInfo.mainPic}]
